...
- Use `workflow-templates/codexkit-parity.yml` for shared skill/provider/Codex parity enforcement through codexkit-owned parity tooling. It auto-skips skill and Codex MCP checks when a repo does not carry those inputs. Use `workflow-templates/mcp-smoke-test.yml` for the lightweight MCP smoke lane before adding repo-local variants.
